...
cur.execute("SELECT * FROM player_fw")
fw_table = cur.fetchall()
fw_df = pd.DataFrame(fw_table, columns=[x[0] for x in cur.description])

# text columns 변환
fw_df['Passes'] = fw_df['Passes'].str.replace(',', '').astype('int64')
fw_df['Shooting accuracy %'] = fw_df['Shooting accuracy %'].str.replace('%', '').astype('int64')
...
3. 모델 조정
columns = ['Appearances', 'Goals', 'Assists', 'Big chances missed']
"""

train, test = train_test_split(fw_df, test_size=0.2, random_state=7)
train, val = train_test_split(train, test_size=0.2, random_state=7)

target = 'PFA_Team'
features = ['Appearances', 'Goals', 'Assists', 'Big chances missed']
...
# model pickle로 저장
with open('model_fw.pkl', 'wb') as pickle_file:

...
cur.execute("SELECT * FROM player_gk")
gk_table = cur.fetchall()
gk_df = pd.DataFrame(gk_table, columns=[x[0] for x in cur.description])

# text columns 변환
gk_df['Passes'] = gk_df['Passes'].str.replace(',', '').astype('int64')
...
3. 모델 조정
columns = ['Appearances', 'Clean sheets', 'Accurate long balls', 'Own goals']
"""

train, test = train_test_split(gk_df, test_size=0.2, random_state=7)
train, val = train_test_split(train, test_size=0.2, random_state=7)

target = 'PFA_Team'
features = ['Appearances', 'Clean sheets', 'Accurate long balls', 'Own goals']
...
ads = ADASYN(random_state=7, n_neighbors=1)
...
# model pickle에 저장
with open('model_gk.pkl', 'wb') as pickle_file:

...
cur.execute("SELECT * FROM player_df")
df_table = cur.fetchall()
df_df = pd.DataFrame(df_table, columns=[x[0] for x in cur.description])

# text columns 변환
df_df['Passes'] = df_df['Passes'].str.replace(',', '').astype('int64')
df_df['Tackle success %'] = df_df['Tackle success %'].str.replace('%', '').astype('int64')
df_df['Cross accuracy %'] = df_df['Cross accuracy %'].str.replace('%', '').astype('int64')
...
3. 모델 조정
columns = ['Appearances', 'Blocked shots', 'Passes per match', 'Passes']
"""
train, test = train_test_split(df_df, test_size=0.2, random_state=7)
train, val = train_test_split(train, test_size=0.2, random_state=7)

target = 'PFA_Team'
features = ['Appearances', 'Blocked shots', 'Passes per match', 'Passes']
...
# model pickle에 저장
with open('model_df.pkl', 'wb') as pickle_file:
...
if __name__ == "__main__":
    ...
    get_score_list(y_test, y_pred, custom_thr)